...
- `/analyze` の統合出力は **`predictions/<race_id>-<slug>.md` に保存し、発走前に git commit**（＝予想の確定）
- レース後は **`/review-race <race_id>`**: `fetch_results.py` → `score_prediction.py --append`（機械採点）→
  外したら `docs/postmortems/` に教訓＋反映先を記録
- **ルックアヘッド汚染ガード（重要）**: バックフィルした過去レースへの「予想練習」「回収率バックテスト」は
  禁止（LLM はカットオフ以前の結果を学習済みのため後知恵が混入する）。校正の母集団は
  「カットオフ後かつ発走前確定」の予想のみ。詳細は `predictions/README.md`
- 公開リポジトリに何をコミットしてよいかは **`docs/data-policy.md`** の分類表に従う
